...
func (_m *DatadogSecuritySignalsAPI) SearchSignals(query string) (<-chan v2datadog.SecurityMonitoringSignal, error) {
	ret := _m.Called(query)

	var r0 <-chan v2datadog.SecurityMonitoringSignal
	if rf, ok := ret.Get(0).(func(string) <-chan v2datadog.SecurityMonitoringSignal); ok {
		r0 = rf(query)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(<-chan v2datadog.SecurityMonitoringSignal)
		}
	}
